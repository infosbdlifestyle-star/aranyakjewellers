          ...
          <div className="animate-marquee flex items-center">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="flex items-center space-x-8 mx-8">
                {(settings['marquee_text'] || '100% BIS Hallmarked | Certified Diamonds | Legacy Since 1995').split('|').map((text, j) => (
                  <React.Fragment key={j}>
                    <span className="text-[10px] tracking-[0.3em] font-bold text-white/80 uppercase">{text.trim()}</span>
        ...
        <div className="container mx-auto px-6 max-w-[1400px] relative z-10">
          ...
          <motion.div style={{ y: yParallaxGrid }} className="grid grid-cols-1 md:grid-cols-12 auto-rows-[500px] md:auto-rows-[600px]">
            ...
            {categories.slice(0, 3).map((cat, idx) => (
              <Link key={cat.id} href={`/category/${cat.slug}`} className={`${idx === 0 ? 'md:col-span-7' : idx === 1 ? 'md:col-span-5 border-t border-r border-b' : 'md:col-span-4 border-l border-b border-r md:border-r-0'} row-span-1 relative group overflow-hidden bg-[#0A0505] flex items-center justify-center p-12 border-white/10 transition-all duration-1000 border`}>
                <MouseParallaxImage src={cat.imageUrl || '/hero-necklace.png'} alt={cat.name} intensity={15} />
                ...
                <div className="relative z-10 text-center flex flex-col items-center justify-end h-full pb-8">
                  {idx === 0 && <span className="text-[9px] tracking-[0.5em] uppercase text-white/70 mb-6 opacity-0 group-hover:opacity-100 transition-all duration-1000 transform translate-y-4 group-hover:translate-y-0">Bespoke</span>}
                  <h4 className="text-4xl md:text-5xl font-serif font-light text-white mb-6">{cat.name}</h4>
