...
const Footer = ({ categories = [], settings = {} }: { categories?: any[], settings?: any }) => {
      ...
      <div className="container mx-auto px-4">
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-white/70">
              {categories.slice(0, 4).map(cat => (
                <li key={cat.id}><Link href={`/category/${cat.slug}`} className="hover:text-secondary transition-colors">{cat.name}</Link></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="space-y-4 text-sm text-white/70">
              <p className="flex items-start gap-3">
                ...
                M B B CHOWMUHANI, AGARTALA 799001
              </p>
              <p className="flex items-center gap-3">
                ...
                {settings['contact_phone'] || '+91-XXXXXXXXXX'}
              </p>
              <a 
                href={`https://wa.me/${(settings['contact_phone'] || '91XXXXXXXXXX').replace(/\D/g,'')}`} 
                target="_blank" 
        ...
        <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row items-center justify-between text-[10px] text-white/40 uppercase tracking-widest">
          ...
          <div className="flex space-x-6 mt-4 md:mt-0">
            ...
            <Link href="/login" className="hover:text-white/70 inline-flex items-center gap-1.5"><svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>Admin Panel</Link>
          </div>
        </div>
      </div>
    </footer>
